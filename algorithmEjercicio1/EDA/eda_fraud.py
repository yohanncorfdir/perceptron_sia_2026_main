...
# =============================================================
# 2. SCATTERPLOT: fraude vs no fraude segun las 2 mejores features
# =============================================================
fraud    = df[df[target] == 1]
no_fraud = df[df[target] == 0]

fig, ax = plt.subplots(figsize=(9, 6))
ax.scatter(no_fraud[feat1], no_fraud[feat2],
           alpha=0.4, s=18, color='steelblue', label='No fraude (0)')
ax.scatter(fraud[feat1], fraud[feat2],
           alpha=0.6, s=18, color='crimson', label='Fraude (1)')
ax.set_xlabel(feat1, fontsize=11)
ax.set_ylabel(feat2, fontsize=11)
ax.set_title(f"Scatterplot: {feat1} vs {feat2}\n(coloreado por {target})", fontsize=13)
